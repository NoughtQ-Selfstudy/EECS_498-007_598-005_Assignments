...
def svm_loss_vectorized(
    ...
    # X = X.to(W.dtype)  
    scores = X.mm(W)                                                         # N * C
    correct_class_score = scores[torch.arange(num_train), y]                           # N * 1
    correct_class_score_view = correct_class_score.view(-1, 1).expand(-1, num_classes)  # N * C
    margin = scores - correct_class_score_view + 1                                      # N * C
    margin[torch.arange(num_train), y] = 0
    loss = margin[margin > 0].sum() / num_train + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    dW = X.T.mm(mask) / num_train + 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates += [.8e-2, .9e-2, 1e-2, 1.1e-2, 1.2e-2]
    regularization_strengths += [.8e-3, .9e-3, 1e-3, 1.1e-3, 1.2e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.max()
        exp_sum = torch.sum(torch.exp(scores))
        probs = torch.div(torch.exp(scores), exp_sum)
        correct_score = scores[y[i]]
        loss += -correct_score + torch.log(torch.sum(torch.exp(scores)))

        gt = torch.zeros_like(scores)
        gt[y[i]] = 1
        dW += torch.outer(X[i], probs - gt)
...
def softmax_get_search_params():
    ...
    regularization_strengths += [1e-3, 5e-4, 1e-4, 5e-5, 1e-5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
