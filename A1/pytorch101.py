...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for num in range(start, stop + 1):
        if num % 10 == 0:
            tmp_seq.append(num)
    if len(tmp_seq) != 0:
        x = torch.tensor(tmp_seq).double()
    else:
        x = torch.empty(0).double()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x_clone = x.clone()
    idx = torch.arange(x_clone.shape[0])
    x_clone[idx, torch.argmin(x, dim=1)] = 0
    y = x_clone
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    m = torch.sum(x, dim=0) / x.shape[0]
    sigma = torch.sqrt(torch.sum(torch.pow(x - m, 2), dim=0) / (x.shape[0] - 1))
    y = (x - m) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
